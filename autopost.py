...
# Reader - lets you iterate through a folder and a file
...
time_iterator = 0
week = "10"
while time_iterator <= 2:
    time = time_iteration(time_iterator)
    time_title = time_indicator(time)
    title = f"Week {week}, {time_title}"
    file_path = config.file_path
    directory_path = f'{file_path}\W{week}{time}'
    # ==================================================
    ...
    res = requests.post(url, headers=payload, data=params, stream=True)
    print("RES SENT")
    print(res)
    print(res.json())
    print("============ Game Posted ============")
